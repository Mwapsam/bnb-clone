...
ActiveRecord::Base.transaction do
  ...
  (0...num_users).each do |idx|
    ...
    if idx < 9
      u = User.all[idx]
      file = open("https://nomad-pass.s3.amazonaws.com/photos/users/0#{idx+1}-user.jpg")
      u.photo.attach(io: file, filename: "0#{idx+1}-user.jpg")
      u.save!
    else
      u = User.all[idx]
      file = open("https://nomad-pass.s3.amazonaws.com/photos/users/#{idx+1}-user.jpg")  
  ...
  (0...num_listings).each do |idx|
    (0...8).each do |photo_idx|
      ...
      if idx < 9
        # Full size Image
        file = open("https://nomad-pass.s3.amazonaws.com/photos/listings/SF/0#{idx+1}-SF/original/0#{idx+1}-0#{photo_idx+1}-SF.jpg")  
        listing.photos.attach(io: file, filename: "0#{idx+1}-0#{photo_idx+1}-SF.jpg")
        
        # Thumbnail
        file = open("https://nomad-pass.s3.amazonaws.com/photos/listings/SF/0#{idx+1}-SF/original/0#{idx+1}-0#{photo_idx+1}-SF.jpg")  
        listing.thumbnails.attach(io: file, filename: "0#{idx+1}-0#{photo_idx+1}-SF.jpg")
      else
        # Full size Image
        file = open("https://nomad-pass.s3.amazonaws.com/photos/listings/SF/#{idx+1}-SF/original/#{idx+1}-0#{photo_idx+1}-SF.jpg")  
        listing.photos.attach(io: file, filename: "#{idx+1}-0#{photo_idx+1}-SF.jpg")

        # Thumbnail
        file = open("https://nomad-pass.s3.amazonaws.com/photos/listings/SF/#{idx+1}-SF/original/#{idx+1}-0#{photo_idx+1}-SF.jpg")
        listing.thumbnails.attach(io: file, filename: "#{idx+1}-0#{photo_idx+1}-SF.jpg")
      end
